const MODELS = [
  { id: "needle3", label: "Needle 3 (2 layers)", base: "/models/needle3/", weights: "needle3.cact" },
  { id: "needle2", label: "Needle 2", base: "/models/needle2/", weights: "needle2.cact" },
];
...
async function runBenchmark() {
  ...
  try {
    for (const model of MODELS) {
      ...
      const peakValues = result.runs.map((run) => Number(run.peak_ram_mb)).filter((value) => Number.isFinite(value) && value > 0);
      setCard(model.id, "decode", formatRate(decode));
      setCard(model.id, "prefill", formatRate(prefill));
      setCard(model.id, "peak", formatMb(peakValues.length ? Math.max(...peakValues) : null));
